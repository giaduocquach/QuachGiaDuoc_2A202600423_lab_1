...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<42} | {'GPT-4o Response':<42} | {'Mini Response':<42} | {'GPT-4o Latency':>14} | {'Mini Latency':>12}"
    separator = "-" * len(header)
    lines = [header, separator]
    for r in results:
        prompt = str(r.get('prompt', ''))[:40]
        gpt4o = str(r.get('gpt4o_response', ''))[:40]
        mini = str(r.get('mini_response', ''))[:40]
        g_lat = f"{r.get('gpt4o_latency', 0):.3f}s"
        m_lat = f"{r.get('mini_latency', 0):.3f}s"
        lines.append(f"{prompt:<42} | {gpt4o:<42} | {mini:<42} | {g_lat:>14} | {m_lat:>12}")
    return "\n".join(lines)
